			...
			listeners : {
				...
				render : {
					fn : function() {
						...
						var divTmpChild = '<div id="'+placeHolderId+'"></div>';
//						em.createChild({
//							tag : 'div',
//							id : placeHolderId
//						});
						Ext.DomHelper.insertHtml('afterBegin',em.dom,divTmpChild);
						var settings = {
	...
	formatProgressBar : function(_v, cellmeta, record) {
		...
		switch (record.data.fileState) {
			case SWFUpload.FILE_STATUS.COMPLETE :
				if (Ext.isIE) {
					returnValue = '<div class="x-progress-wrap" style="height:20px;line-height:18px;width:80px;">'
							+ '<div class="x-progress-inner">'
							+ '<div style="width: 89%;" class="x-progress-bar x-progress-text">' + '100 %'
					'</div>' + '</div>' + '</div>';
				} else {
					returnValue = '<div class="x-progress-wrap" style="height:20px;line-height:18px;width:80px;">'
							+ '<div class="x-progress-inner">' + '<div id="progressBar_' + record.data.fileId
							+ '" style="width:89%;" class="x-progress-bar">' + '</div>' + '<div id="progressText_'
							+ record.data.fileId
							+ '" style="width: 90%;" class="x-progress-text x-progress-text-back" />100 %</div>'
					'</div>' + '</div>';
				}
				break;
			default :
				returnValue = '<div class="x-progress-wrap" style="height:20px;line-height:18px;width:80px;">' + '<div class="x-progress-inner">'
						+ '<div id="progressBar_' + record.data.fileId + '" style="width: 0%;" class="x-progress-bar">'
						+ '</div>' + '<div id="progressText_' + record.data.fileId
						+ '" style="width:89%;" class="x-progress-text x-progress-text-back" />0 %</div>'
				'</div>' + '</div>';
				break;
		}
		return returnValue;
	...
	onUploadSuccess : function(file, serverData) {
		...
		if (jsonData.success) {
			UploadPanel.ImageArr.push(jsonData.path);
			var record = thiz.fileList.getStore().getById(Ext.getDom('fileId_' + file.id).parentNode.id);
			record.set('fileState', file.filestatus);
			record.set('filename',jsonData.path);//返回的已上传的文件地址
			record.set('oldfilename',file.name.substring(0,file.name.length-4));
			record.commit();
		}
